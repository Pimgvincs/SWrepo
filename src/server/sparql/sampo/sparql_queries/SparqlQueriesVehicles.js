...
export const vehicleInstanceProperties = `
    {
      ?id a voc:Vehicle .
      BIND(?id AS ?uri__id)
      ...
      BIND(CONCAT("/vehicle/page/", REPLACE(STR(?id), "^.*[/#]([^/#]+)$", "$1")) AS ?uri__dataProviderUrl)
    }
    UNION
    {
    ...
    UNION
    {
      ?id voc:vehicleClass ?temp_vehicleClass .
      BIND(STR(?temp_vehicleClass) AS ?vehicleClass__prefLabel)
      BIND(STR(?temp_vehicleClass) AS ?vehicleClass__id)
    }
`